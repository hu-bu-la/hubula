...
//New Dao 实例化公共方法
func NewAreaDao(engine *xorm.Engine) *AreaDao {
	return &AreaDao{
		engine: engine,
	}
}
...
// NewAdmin 初始化
func (d *AreaDao) newModel() *models.Area {
	return new(models.Area)
}

// newMakeDataArr 初始化列表
func (d *AreaDao) newMakeDataArr() []models.Area {
...
func (d *AreaDao) GetAll(q map[string]interface{}, fields []string, orderBy string, page int, limit int) (*datasource.Paginator, error) {
	session := datasource.Filter(q)
	defer session.Close()
	count, err := session.Count(&models.Area{})
	if err != nil {
		fmt.Println(err)
...
// GetById 获取单条记录
func (d *AreaDao) GetById(id int) (*models.Area, error) {
	m := d.newModel()
	m.Id = id
...
// Create 添加单条记录
func (d *AreaDao) Create(data *models.Area) (int64,error) {
	num, err := d.engine.InsertOne(data)
	return num,err
}

// Update 修改单条记录
func (d *AreaDao) Update(data *models.Area, columns []string) (int64,error) {
	num, err := d.engine.ID(data.Id).MustCols(columns...).Update(data)
	return num,err
}

// RuanDelete 软删除单条记录
func (d *AreaDao) RuanDelete(id int) (int64, error) {
	m := d.newModel()
	m.Id = id
	m.Sort = 0
...
// Delete 删除单条记录
func (d *AreaDao) Delete(id int) (int64, error) {
	m := d.newModel()
	m.Id = id
...
// GetWhere Sql语句
func (d *AreaDao) GetWhere(sql string) []models.Area {
