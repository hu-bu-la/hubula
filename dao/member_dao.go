...
func (d *MemberDao) GetAll(q map[string]interface{}, fields []string, orderBy string, page int, limit int) (*datasource.Paginator, error) {
	session := datasource.Filter(q)
	defer session.Close()
	count, err := session.Count(&models.Member{})
	if err != nil {
		fmt.Println(err)
...
// GetById 获取单条记录
func (d *MemberDao) GetById(id int) (*models.Member, error) {
	m := d.newModel()
	m.Uid = id
...
// RuanDelete 软删除单条记录
func (d *MemberDao) RuanDelete(id int) (int64, error) {
	m := d.newModel()
	m.Uid = id
	m.IsDel = 0
...
// Delete 删除单条记录
func (d *MemberDao) Delete(id int) (int64, error) {
	m := d.newModel()
	m.Uid = id
